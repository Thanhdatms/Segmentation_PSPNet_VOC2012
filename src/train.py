...
import time
from dataset import make_data_path, MyDataSet, DataTransform
import pandas as pd
...
optimizer = optim.SGD([
    ...
    {"params": model.feature_residual_1.parameters(), 'lr': 1e-3},
    {"params": model.feature_residual_2.parameters(), 'lr': 1e-3},
    {"params": model.feature_dilated_residual_1.parameters(), 'lr': 1e-3},
    {"params": model.feature_dilated_residual_2.parameters(), 'lr': 1e-3},
    {"params": model.pyramid_pooling.parameters(), 'lr': 1e-3},
    {"params": model.decode_feature.parameters(), 'lr': 1e-2},
    {"params": model.aux_loss.parameters(), 'lr': 1e-2},
...
def train_model(model, dataloader_dict, criterion, scheduler, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        pd.DataFrame(logs).to_csv('checkpoints/logs.csv', index=False)
        if epoch > 30:
            
            filename = f'pspnet50_{epoch}_loss_{epoch_val_loss/num_val_imgs}.pth'
...
if __name__ == '__main__':
    ...
    train_img_list, train_anno_list, val_img_list, val_anno_list = make_data_path(rootpath)
